...
/**
 * La feuille de compétence est basée sur la classe ItemSheet
 * Feuille de compétence affichée à partir de la feuille de perso pour modifier en profondeur les compétences
 * @class
 */
export class RdDFeuilleCptcPJ extends ItemSheet {
	/**
	 * Étend et remplace les options d'affichage par défaut
	 * @function defaultOptions
	 * @returns {object}
	 ...
	 */
	static get defaultOptions() {
		console.log(`RdD | RdDFeuilleCptcPJ.defaultOptions`);
		/**
		 * @property {string} options.classes classes CSS appliquées à la feuille
		 * @property {string} options.template chemin d'accès au squelette HTML de la feuille
		 * @property {number} options.width largeur de la fenêtre
		...
		return mergeObject(super.defaultOptions, {
			classes: ["RdD", "feuille", "item", "cptc"],
			template: "systems/RdD/templates/feuille-cptc-pj.html",
			width: 450,
			height: 300
		});
	}
	...
	getData() {
		let data = super.getData();
		console.log(`RdD | RdDFeuilleCptcPJ.getData ${JSON.stringify(data)}`);
		return data;
	}
	 ...
	 */
	activateListeners(html) {
		console.log(`RdD | RdDFeuilleCptcPJ.activateListeners`);
		super.activateListeners(html);
	...
	_onChangeInput(event) {
		...
		const id = input.id;
		const classList = input.classList;
		console.log(`RdD | RdDFeuilleCptcPJ._onChangeInput ${id}, ${classList[0]}`);
		...
		try {
			...
			console.log("RdD | RdDFeuilleCptcPJ._onChangeInput – Appel _onSubmit");
			this._onSubmit(event);
		} catch (erreur) {
			console.log(`RdD | RdDFeuilleCptcPJ._onChangeInput ${erreur}`);
		}
	}
	 ...
	 */
	async _onSubmit(event, {updateData=null, preventClose=false}={}) {
		console.log(`RdD | RdDFeuilleCptcPJ._onSubmit`);
		const form = this.element.find("form").first()[0];
		if (form.querySelector(".erreur")) {
			console.log("RdD | RdDFeuilleCptcPJ._onSubmit – erreur");
			ui.notifications.error(game.i18n.localize("RdD.erreurs.pasDeMàJ"));
		}
		else {
			console.log(`RdD | RdDFeuilleCptcPJ._onSubmit – pas d'erreur ${JSON.stringify(updateData)}`);
			super._onSubmit(event, {updateData, preventClose});
		}
	}
	 ...
	 */
	async _updateObject(event, formData) {
		console.log(`RdD | RdDFeuilleCptcPJ._updateObject ${JSON.stringify(formData)}`);
		super._updateObject(event, formData);
	}	
}